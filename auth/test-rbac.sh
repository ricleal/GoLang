...
echo -e "${YELLOW}1. Testing regular user (alice - role: user)${NC}"
echo "---------------------------------------------"
...
echo ""
echo -e "${YELLOW}2. Alice accessing /api/v1/cowsay (should succeed)${NC}"
...
if echo "$RESPONSE" | grep -q "cow"; then
  echo -e "${GREEN}✅ Alice can access cowsay endpoint${NC}"
  echo "   Service: $(echo $RESPONSE | jq -r '.service')"
else
  echo -e "${RED}❌ Alice cannot access cowsay endpoint${NC}"
fi

# Test 3: Alice tries to access admin endpoint (should fail)
echo ""
echo -e "${YELLOW}3. Alice accessing /api/v1/admin (should be FORBIDDEN)${NC}"
RESPONSE=$(curl -s -w "\n%{http_code}" http://localhost:8000/api/v1/admin \
  -H "Authorization: Bearer $ALICE_TOKEN")
  
STATUS=$(echo "$RESPONSE" | tail -n1)
if [ "$STATUS" = "403" ]; then
  echo -e "${GREEN}✅ Alice correctly denied access (403 Forbidden)${NC}"
else
...
echo ""
echo -e "${YELLOW}4. Testing admin user (admin - role: admin)${NC}"
echo "--------------------------------------------"
...
echo ""
echo -e "${YELLOW}5. Admin accessing /api/v1/cowsay (should succeed)${NC}"
RESPONSE=$(curl -s -X POST http://localhost:8000/api/v1/cowsay \
...
if echo "$RESPONSE" | grep -q "cow"; then
  echo -e "${GREEN}✅ Admin can access cowsay endpoint${NC}"
  echo "   Service: $(echo $RESPONSE | jq -r '.service')"
else
  echo -e "${RED}❌ Admin cannot access cowsay endpoint${NC}"
fi

# Test 6: Admin accesses admin endpoint (should work)
echo ""
echo -e "${YELLOW}6. Admin accessing /api/v1/admin (should succeed)${NC}"
RESPONSE=$(curl -s -w "\n%{http_code}" http://localhost:8000/api/v1/admin \
  -H "Authorization: Bearer $ADMIN_TOKEN")
...
if [ "$STATUS" = "200" ]; then
  ...
  echo ""
  echo -e "${YELLOW}Admin Panel Response:${NC}"
  echo "$BODY" | jq '.'
else
...
echo ""
echo -e "${YELLOW}7. Testing another regular user (bob - role: user)${NC}"
echo "--------------------------------------------------"
...
echo ""
echo -e "${YELLOW}8. Bob accessing /api/v1/admin (should be FORBIDDEN)${NC}"
RESPONSE=$(curl -s -w "\n%{http_code}" http://localhost:8000/api/v1/admin \
  -H "Authorization: Bearer $BOB_TOKEN")
  
STATUS=$(echo "$RESPONSE" | tail -n1)
if [ "$STATUS" = "403" ]; then
  echo -e "${GREEN}✅ Bob correctly denied access (403 Forbidden)${NC}"
else
  echo -e "${RED}❌ Expected 403, got: $STATUS${NC}"
fi
