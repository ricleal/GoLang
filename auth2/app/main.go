...
// Middleware to verify Envoy headers (authentication already done by Envoy)
func verifyEnvoyHeaders(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		username := r.Header.Get("X-Username")
		role := r.Header.Get("X-Role")

		if username == "" || role == "" {
			log.Printf("[%s/%s] Request denied: missing Envoy headers", appName, hostname)
...
func requireRole(role string, next http.HandlerFunc) http.HandlerFunc {
	return verifyEnvoyHeaders(func(w http.ResponseWriter, r *http.Request) {
...
func infoHandler(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]string{
		...
		"instance": hostname,
		"version":  "1.0.0",
		"note":     "Authentication handled by Envoy Gateway",
	})
}

func main() {
	http.HandleFunc("/api/v1/cowsay", verifyEnvoyHeaders(cowsayHandler))
	...
	log.Printf("Authentication: Trusting Envoy Gateway headers")
